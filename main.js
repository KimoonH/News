...
const getLatesNews = async () => {
    ...
    let PAGE_SIZE = 20
    let PAGE = 1;
    const url = new URL(`http://times-node-env.eba-appvq3ef.ap-northeast-2.elasticbeanstalk.com/top-headlines?q=${Keyword}&country=kr&pageSize=${PAGE_SIZE}&page=${PAGE}`) // 개발자 필요로 하는 함수를 제공해주는데, URL()함수를 만듬. url인스턴스라고 부름. url에 필요한 함수와 변수들을 제공. 새로운 URL인스턴스를 만들어준다.
